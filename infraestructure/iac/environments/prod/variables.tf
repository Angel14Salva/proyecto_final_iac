...
variable "manage_apigw_account_settings" {
  description = "aws_api_gateway_account es un recurso a nivel de cuenta/region -- solo UN entorno de los desplegados en la misma cuenta/region debe tenerlo en true (hoy: dev). prod lo deja en false para no chocar."
  type        = bool
  default     = false
}

variable "manage_config_recorder" {
  description = "aws_config_configuration_recorder es un recurso a nivel de cuenta/region (AWS solo permite UNO por cuenta/region) -- solo UN entorno debe tenerlo en true (hoy: dev). prod lo deja en false para no chocar."
  type        = bool
  default     = false
}

variable "manage_oidc_provider" {
  description = "aws_iam_openid_connect_provider es unico por URL dentro de una cuenta AWS -- solo UN entorno debe tenerlo en true (hoy: dev). prod lo deja en false para no chocar."
  type        = bool
  default     = false
}
